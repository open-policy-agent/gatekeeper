...
func TestDeriveK8sVersionRange(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// no single cluster can satisfy both, so the range is an error rather
			// than empty (which would read as "compatible with everything").
			name: "contradictory range errors",
			kinds: []groupKind{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotMin, gotMax, err := deriveK8sVersionRange(tt.kinds)
...
`,
		...
		{
			// An omitted apiGroups matches every group (wildcard), not just the
			// core group, so it carries no usable lifecycle signal and is skipped.
			name: "missing apiGroups is treated as a wildcard and skipped",
			yaml: `apiVersion: constraints.gatekeeper.sh/v1beta1
kind: K8sTest
spec:
  match:
...
// TestTargetGroupKinds_DirectConstraintFile verifies the samples/constraint.yaml
// layout (a constraint stored directly under samples/, not in a per-sample
// subdirectory) still contributes derived bounds. This layout is used by the
// generator test fixture and some library policies.
func TestTargetGroupKinds_DirectConstraintFile(t *testing.T) {
	templateDir := t.TempDir()
  ...
  match:
    kinds:
      - apiGroups: ["networking.k8s.io"]
        kinds: ["Ingress"]
`
	if err := os.WriteFile(filepath.Join(samplesDir, "constraint.yaml"), []byte(constraint), 0o600); err != nil {
		t.Fatal(err)
	}
...
`
	...
	t.Run("derives min and max from removed resource", func(t *testing.T) {
		...
		if policy.MaxKubernetesVersion != "v1.21" {
			t.Errorf("MaxKubernetesVersion: got %q, want %q", policy.MaxKubernetesVersion, "v1.21")
		}
	})

...
func TestReconcileSkipsSecondIdenticalStatusUpdate(t *testing.T) {
	...
	provider := &externaldatav1beta1.Provider{
		...
		Spec: externaldatav1beta1.ProviderSpec{URL: "https://provider.example.com"},
	}
	podStatus := &statusv1beta1.ProviderPodStatus{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "pod-a-provider",
			Namespace: util.GetNamespace(),
			Labels: map[string]string{
				statusv1beta1.ProviderNameLabel: provider.Name,
			},
		},
	...
	request := reconcile.Request{NamespacedName: types.NamespacedName{Name: provider.Name}}

	_, err := reconciler.Reconcile(ctx, request)
	require.NoError(t, err)
	require.Equal(t, 1, statusClient.updateCount, "first reconcile should self-heal missing aggregate status")

	updated := &externaldatav1beta1.Provider{}

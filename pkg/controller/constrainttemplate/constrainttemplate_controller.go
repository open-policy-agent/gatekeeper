...
func (a *Adder) Add(mgr manager.Manager) error {
	...
	r, err := newReconciler(mgr, a.CFClient, a.WatchManager, a.Tracker, constraintEvents, constraintEvents, a.GetPod, webhookCache)
	if err != nil {
		return err
	}
...
func (r *ReconcileConstraintTemplate) manageVAP(ctx context.Context, ct *v1beta1.ConstraintTemplate, unversionedCT *templates.ConstraintTemplate, status *statusv1beta1.ConstraintTemplatePodStatus, logger logr.Logger, generateVap bool) error {
	...
	if generateVap && isVapAPIEnabled && groupVersion != nil {
		...
		if r.webhookCache != nil {
			config, exists := r.webhookCache.GetConfig("validation.gatekeeper.sh")
			if exists {
		...
		if webhookConfig != nil {
			...
			transformedVap, err = transform.TemplateToPolicyDefinitionWithWebhookConfig(unversionedCT, webhookConfig)
		} else {

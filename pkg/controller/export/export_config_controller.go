...
func (r *Reconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if len(cfg.Data) == 0 {
		return reconcile.Result{}, fmt.Errorf("data missing in configmap, unable to configure exporter")
	}
	if _, ok := cfg.Data["driver"]; !ok {
		return reconcile.Result{}, fmt.Errorf("missing driver field in configmap, unable to configure exporter",)
	}

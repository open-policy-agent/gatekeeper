...
Starting with v3.13+, Gatekeeper supports caching of responses from external data providers for audit and the validating webhook. Mutation provider responses are also cached when external data is used by mutation. Gatekeeper caches the response based on the `Key` and `Value` received as part of the [`ProviderResponse`](#providerresponse). By default, the cache is invalidated after 3 minutes, which is the default Time-to-Live (TTL). You can configure the TTL using the `--external-data-provider-response-cache-ttl` flag. Setting the flag to 0 disables this cache.

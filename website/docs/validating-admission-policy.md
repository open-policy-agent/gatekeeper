...
If a K8sNativeValidation source omits `failurePolicy`, Gatekeeper uses `--default-k8s-native-validation-failure-policy`, which defaults to `Fail`, for both Gatekeeper's CEL evaluation and generated VAP resources. This is separate from `validatingWebhookFailurePolicy`, which controls how Kubernetes handles failures when calling Gatekeeper's validating webhook and does not set CEL or generated VAP failure policy defaults.

...
If a K8sNativeValidation source omits `failurePolicy`, Gatekeeper uses `--default-failure-policy-for-cel-engine`, which defaults to `Fail`, for both Gatekeeper's CEL evaluation and generated VAP resources.

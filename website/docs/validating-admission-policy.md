...
Shared-policy cleanup runs in a separate worker on generator pods, in batches of up to 100 templates. Each batch shares one paginated API list when cached references cannot establish that policies are still in use. List failures leave all candidates intact; deletion failures are retried with backoff. Ownership and resource-version checks protect replaced policies. Cleanup does not block template reconciliation or run in the admission request path. The worker does not reuse negative reference results between batches.

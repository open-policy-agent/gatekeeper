...
2. To upgrade or install Gatekeeper with `--export-violations` set to `true`, `--audit-connection` set to `audit-connection`, `--audit-channel` set to `audit-channel` on audit pod.
    ...
    --set audit.exportViolations=true \

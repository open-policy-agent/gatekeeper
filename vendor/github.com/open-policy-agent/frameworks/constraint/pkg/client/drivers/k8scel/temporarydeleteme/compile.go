...
// forked from k8s.io/apiserver/pkg/admission/plugin/cel/compile.go @ Kubernetes version 1.27.2

//nolint:all do not lint K8s code
package temporarydeleteme
...
import (
	...
	originalCEL "k8s.io/apiserver/pkg/admission/plugin/cel"
...
func buildRequiredVarsEnv() (*cel.Env, error) {
	...
	// BEGIN forked code.
	propDecls = append(propDecls, cel.Variable(VariablesName, cel.DynType))
	// END forked code.
...
type envs map[originalCEL.OptionalVariableDeclarations]*cel.Env

func buildEnvWithVars(baseVarsEnv *cel.Env, options originalCEL.OptionalVariableDeclarations) (*cel.Env, error) {
	var opts []cel.EnvOption
...
func buildWithOptionalVarsEnvs(requiredVarsEnv *cel.Env) (envs, error) {
	...
	for _, hasParams := range []bool{false, true} {
		for _, hasAuthorizer := range []bool{false, true} {
			opts := originalCEL.OptionalVariableDeclarations{HasParams: hasParams, HasAuthorizer: hasAuthorizer}
...
func CompileCELExpression(expressionAccessor originalCEL.ExpressionAccessor, optionalVars originalCEL.OptionalVariableDeclarations, perCallLimit uint64) originalCEL.CompilationResult {
	var env *cel.Env
	envs, err := getEnvs()
	if err != nil {
		return originalCEL.CompilationResult{

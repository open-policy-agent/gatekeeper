...
func runUpgrade(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		if upgradeDryRun {
			return fmt.Errorf("%w (upgrade --dry-run still requires cluster access to read installed policy versions)", err)
		}
		return err
	}
	...
	if len(result.Incompatible) > 0 {
		return gatorpolicy.NewPartialSuccessError(fmt.Sprintf("upgrade incomplete: %d policies skipped (incompatible Kubernetes version, use --force to override)",
